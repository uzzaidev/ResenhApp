...
#### 4.2.6 Sistema de Modalidades Esportivas ✅ **IMPLEMENTADO** (V2.0)
...
CREATE TABLE sport_modalities (
  ...
  name VARCHAR(50) NOT NULL, -- 'Futebol', 'Vôlei', 'Basquete'
  description TEXT,
...
**Funcionalidades:**
- Grupos podem ter múltiplas modalidades
- Cada modalidade pode ter configurações específicas
- Suporte para atléticas (múltiplas modalidades) vs peladas (uma modalidade)

**Relacionamentos:**
- `sport_modalities.group_id` → `groups.id`
- `athlete_modalities.modality_id` → `sport_modalities.id`
- `events.modality_id` → `sport_modalities.id`

**Funções:**
- `get_group_modalities(p_group_id UUID)` - Lista modalidades de um grupo

---

#### 4.2.7 Sistema de Créditos ✅ **IMPLEMENTADO**
...
- `credits_purchased INTEGER` - Total comprado (lifetime)
- `credits_consumed INTEGER` - Total consumido (lifetime)
- `pix_code TEXT` - Código Pix para pagamentos

**Sistema de Créditos:**
- Grupos compram créditos para usar features premium
- Features consomem créditos automaticamente
- Histórico completo de transações

**Features Premium (custos):**
...
**Funções:**
- `consume_credits(p_group_id, p_amount, p_feature, p_user_id, p_event_id)` - Consome créditos
- `add_credits(p_group_id, p_amount, p_user_id, p_package_id)` - Adiciona créditos (compra)
- `get_pix_code_for_group(p_group_id)` - Retorna código Pix (prioridade: atlética > grupo)

**Pacotes Padrão:**
- Básico: 100 créditos = R$ 20,00
- Intermediário: 300 créditos = R$ 50,00 (economia 10%)
- Premium: 700 créditos = R$ 100,00 (economia 20%)
- Mensal: 200 créditos/mês = R$ 30,00/mês

---

#### 4.2.8 Hierarquia de Grupos ✅ **IMPLEMENTADO**

**Migration:** `20260227000008_hierarchy_and_credits.sql`

**Colunas em `groups`:**
- `parent_group_id UUID` - Referência ao grupo pai (atlética)
- `group_type VARCHAR(20)` - 'athletic' ou 'pelada'
...
**Funcionalidades:**
- Atléticas podem ter múltiplos grupos filhos (peladas)
- Herança de configurações (Pix code, créditos)
- Permissões hierárquicas (admin de atlética gerencia grupos filhos)

**Validações:**
- Trigger `check_hierarchy_cycle()` previne referências circulares
- Máximo de 10 níveis de profundidade
- Não permite auto-referência

**Funções:**
- `get_pix_code_for_group(p_group_id)` - Retorna Pix da atlética (prioridade) ou do grupo
- `can_manage_group(p_user_id, p_group_id)` - Verifica se usuário pode gerenciar (admin do grupo ou da atlética pai)

---

#### 4.2.9 Multi-Modalidades (Futuro)
