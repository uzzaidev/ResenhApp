...
export default async function EventRsvpPage({ params, searchParams }: RouteParams) {
  ...
  const backHref =
    safeReturnTo ||
    (event.event_type === "game" || event.event_type === "match"
      ? "/eventos?tipo=jogo"
      : "/eventos?tipo=treino");
  ...
  // Buscar status atual do usuÃ¡rio neste evento
  ...
  // Buscar membros do grupo para o admin gerenciar confirmaÃ§Ãµes
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-xl border bg-card p-6">
        ...
        <div className="mt-5 max-w-md">
          {maxPlayers > 0 ? (
            <RsvpProgress confirmed={confirmedCount} total={maxPlayers} />
          ) : (
            <p className="text-sm text-muted-foreground">
              {confirmedCount} confirmações (evento sem limite de vagas)
            </p>
          )}
        </div>
      </section>
