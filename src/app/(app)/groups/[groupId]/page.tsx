...
export default async function GroupPage({ params }: RouteParams) {
  ...
  // Buscar prÃ³ximos eventos do grupo
  ...
  // Se houver eventos, buscar estatÃ­sticas
    ...
    try {
      const rankingData = await sql`
        WITH player_games AS (
          -- Eventos que cada jogador jogou (estÃ¡ em um time do evento finalizado)
          SELECT DISTINCT
            tm.user_id,
            t.event_id
          FROM team_members tm
            ...
            {group.user_role === "admin" && (
              <>
                <Button asChild size="sm">
                  <Link href={`/eventos/novo?groupId=${groupId}`}>
                ...
                <Button asChild variant="outline" size="sm">
                  <Link href={`/grupos/${groupId}/configuracoes`}>
